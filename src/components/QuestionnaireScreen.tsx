...
export default function NarrativeQuiz({ initialAnswers, startAtLast, apiError, onSubmit }: Props) {
  ...
  const emailOk = EMAIL_REGEX.test(answers.email)
  const isEnough = isLast ? (textOk && emailOk) : textOk
    ...
    >
      ...
      {isLast && (
        <div style={{ padding: '12px 18px 0' }}>
          <div
            style={{
              border: `1.5px solid ${emailOk ? '#292F56' : '#D8D8D8'}`,
              borderRadius: 14,
              padding: '4px 14px',
              background: '#fff',
              transition: 'border-color .3s',
