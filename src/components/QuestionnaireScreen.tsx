...
function ProgressHeader({ step }: { step: number }) {
  const total = DIMENSION_ORDER.length + 1
  const isEmailStep = step === DIMENSION_ORDER.length
  const cfg = !isEmailStep ? DIMENSION_CONFIGS[DIMENSION_ORDER[step]] : null
  return (
    <div style={{ padding: '14px 24px 16px', background: '#fff' }}>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 14 }}>
        {DIMENSION_ORDER.map((k, i) => (
          <div
            ...
            style={{
              ...
              height: 3,
              borderRadius: 99,
              background: i < step || (i === step && !isEmailStep) ? DOT_COLOR[k] : '#EAEAEA',
        ...
        <div
          style={{
            ...
            height: 3,
            borderRadius: 99,
            background: step >= DIMENSION_ORDER.length ? '#292F56' : '#EAEAEA',
            transition: 'background .35s',
          }}
        />
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'center', alignItems: 'center' }}>
        ...
        <div
          style={{
            width: isEmailStep ? 34 : 24,
            height: isEmailStep ? 34 : 24,
            borderRadius: '50%',
            background: step >= DIMENSION_ORDER.length ? '#292F56' : '#fff',
            border: `1.5px solid ${step >= DIMENSION_ORDER.length ? '#292F56' : '#D8D8D8'}`,
            color: '#fff',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            fontFamily: 'Inter',
            fontWeight: 800,
            fontSize: isEmailStep ? 14 : 11,
            transition: 'all .3s cubic-bezier(.2,.7,.2,1)',
            boxShadow: isEmailStep ? '0 2px 8px rgba(0,0,0,.08)' : 'none',
          }}
        >
          {step > DIMENSION_ORDER.length ? (
            <svg width="10" height="10" viewBox="0 0 10 10">
              <path
                d="M1 5 L4 8 L9 2"
                stroke="#fff"
      ...
      >
        {isEmailStep ? '第 六 題「電子郵件」· 共六題' : `第 ${ORDINAL[step]} 題「${cfg!.label}」· 共五題`}
      </div>
    </div>
  )
}
...
export default function NarrativeQuiz({ initialAnswers, startAtLast, apiError, onSubmit }: Props) {
  const [step, setStep] = useState(startAtLast ? DIMENSION_ORDER.length : 0)
  const [answers, setAnswers] = useState<NarrativeAnswers>(initialAnswers)
  const [showHint, setShowHint] = useState(false)
  const textareaRef = useRef<HTMLTextAreaElement>(null)

  const isEmailStep = step === DIMENSION_ORDER.length
  const key = isEmailStep ? null : DIMENSION_ORDER[step]
  const cfg = !isEmailStep ? DIMENSION_CONFIGS[key!] : null
  const color = !isEmailStep ? DOT_COLOR[key!] : '#292F56'
  const darkGlyph = !isEmailStep ? DARK_GLYPH[key!] : false
  const currentText = isEmailStep ? answers.email : answers[key!]
  const charCount = currentText.length
  const isEnough = isEmailStep ? EMAIL_REGEX.test(answers.email) : charCount >= MIN_CHARS
  const remaining = !isEmailStep ? Math.max(0, MIN_CHARS - charCount) : 0
  const isLast = step === DIMENSION_ORDER.length
    ...
    >
      ...
      {!isEmailStep ? (
        ...
        <div style={{ padding: '20px 24px 0', textAlign: 'center' }}>
          ...
          >
            請留下你的電子郵件，我們將把你的專屬報告和後續練習計畫傳送給你。
          </p>
        </div>
      )}

      {isEmailStep ? (
        <div style={{ padding: '12px 18px 0', flex: 1, display: 'flex', flexDirection: 'column' }}>
          <div
            style={{
              position: 'relative',
              border: `1.5px solid ${isEnough ? '#292F56' : '#D8D8D8'}`,
              borderRadius: 14,
              padding: '14px 12px',
              background: '#fff',
